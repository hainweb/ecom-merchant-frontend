...
const AnalyticsDashboard = ({
  ...
  useEffect(() => {
    const fetchCategoryProducts = () => {
      axios
        .get(`${BASE_URL}/get-products-category`, { withCredentials: true })
        .then((response) => {
    ...
    if (active && payload && payload.length) {
      return (
        <div className="bg-white border border-gray-200 p-2 shadow-lg rounded">
          ...
          <p
            className={`text-${
              payload[0].payload.quantity <= 5
                ? "red-500"
                : payload[0].payload.quantity <= 15
                ? "yellow-500"
                : "green-500"
            }`}
          >
              ...
              <div className="bg-white rounded-lg shadow p-6">
                <div className="flex items-center justify-between mb-4">
                  <h3 className="text-gray-500 text-sm">
                    Total Order including cancled and returned orders
                  </h3>
