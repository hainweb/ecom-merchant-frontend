...
const EditProduct = () => {
  ...
  useEffect(() => {
    const fetchProduct = async () => {
      try {
        const response = await axios.get(`${BASE_URL}/edit-product/${id}`, {
  ...
  const addCustomOption = () => {
    if (newOption.name.trim() && newOption.values.trim()) {
      ...
      setCustomOptions([
        ...customOptions,
        { name: newOption.name, values: valuesArray },
      ]);
  ...
  const validateForm = () => {
    ...
    if (!product.SellingPrice || Number(product.SellingPrice) <= 0) {
      ...
      product.Price &&
      Number(product.SellingPrice) <= Number(product.Price)
    ) {
      ...
      {loading ? (
        ...
        <form onSubmit={handleFormSubmit} className="space-y-6">
          ...
          <div>
            ...
            {errors.Return && (
              <p className="text-red-500 text-sm mt-1">{errors.Return}</p>
            )}
          </div>
          ...
          {saveLoading ? (
            <button
              type="button"
              class="bg-indigo-500 text-white px-4 py-2 flex items-center rounded disabled:opacity-75 cursor-not-allowed"
              disabled
            >
              <svg
            ...
            <button
              ...
              className={`w-full py-3 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 ${
                hasProductChanged()
